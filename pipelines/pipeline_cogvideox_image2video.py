...
from .utils.preprocessors.depth_preprocessor import DepthPreprocessor
from .utils.preprocessors.segmentation_preprocessor import SegmentationPreprocessor
import numpy as np
from PIL import Image
...
class CogVideoXImageToVideoPipeline(DiffusionPipeline, CogVideoXLoraLoaderMixin):
    ...
    def _prepare_loss(self, video, loss_type = None):
        if loss_type not in ["sketch", "gray", "depth", "segmentation"]:
            raise ValueError("loss_type must be one of ['sketch', 'gray', 'depth', 'segmentation']")
                
        if loss_type == "sketch" or loss_type == "gray":
            assert video is not None, "video must be provided when loss_type is 'scribble'"
            ...
            with torch.no_grad():
                video = [self.aug(frame, mode=loss_type) for frame in video]      
        
        elif loss_type == "depth":
            assert video is not None, "video must be provided when loss_type is 'depth'"
            ...
            assert video is not None, "video must be provided when loss_type is 'depth'"
            if not hasattr(self, "segmentation_preprocessor"): 
                self.segmentation_preprocessor = SegmentationPreprocessor.from_pretrained("nvidia/segformer-b0-finetuned-ade-512-512").to(self._execution_device)
            with torch.no_grad():
                video = [self.segmentation_preprocessor(frame, return_class_ids=True, return_type="pt") for frame in video]
    ...
    def __call__(
        ...
        cond_video = self._prepare_loss(video, loss_fn)
        ...
        with self.progress_bar(total=num_inference_steps) as progress_bar:
            for i, t in enumerate(timesteps):
                ...
                for rep in range(n_repeats+1):
                    ...
                    # FreeDoM step with time-travel trick
                    if in_guidance_range and rep < n_repeats:
                        total_loss = 0.0
                        for fixed_frame in fixed_frames:
                            ...
                            latent_key = pred_original_sample[:, latent_frame-2:latent_frame+1]
                            decoded_frames = self.decode_latents(latent_key)
                            del latent_key

                            relative_frame_idx = (fixed_frame - 1) % 4 + 5
                            pred_frame = decoded_frames[:, :, relative_frame_idx:relative_frame_idx+1] # 1, 3, 1, H, W

                            if loss_fn == "sketch" or loss_fn == "gray":
                            ...
                            elif loss_fn == "segmentation":
                                pred_segmentation = self.segmentation_preprocessor(pred_frame.float().squeeze(2), return_logits=True, return_type="pt")
                                target_ids = cond_video[fixed_frame].to(pred_segmentation.device).long().squeeze(1)  # [B,H,W]
                                num_classes = pred_segmentation.shape[1]
                                target_onehot = F.one_hot(target_ids, num_classes).permute(0,3,1,2).float()    # [B,C,H,W]
                                loss = F.mse_loss(
                                    pred_segmentation.float(),
                                    target_onehot,
                                    reduction="mean"
                                )
